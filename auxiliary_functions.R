...
pls_fun = function(response, predictors_train, predictors_test, nbf_x, nbf_y){
  ...
  B_spline_basis_x = create.bspline.basis(c(0,1), nbasis = nbf_x)
  B_spline_basis_funs_x = eval.basis(mtp, B_spline_basis_x)
  
  # B-spline basis for response
  B_spline_basis_y = create.bspline.basis(c(0,1), nbasis = nbf_y)
  ...
  smooth_arg_test = matrix(mtp, nrow = nrow(predictors_test[[1]]), ncol = ncol(predictors_test[[1]]), byrow=T)
  ...
  for(sm in 1:np){
    X_train_smooth[[sm]] = t(smooth.basis(argvals=t(smooth_arg_train), y=t(predictors_train[[sm]]), fdParobj=B_spline_basis_x)$fd$coefs)
  ...
  for(forw1 in 2:np){
    ...
    for(forw in 1:ncol(error_for_forward_selection)){
      ...
      error_for_forward_selection[2,forw] = mean((approx_response - predicted_curves_forw_model)^2)
    }
